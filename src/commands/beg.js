...
module.exports = {
  ...
  async execute(msg, args, client) {
    if (msg.mentions.users.first() == undefined) {
      return msg.channel.send(`乞討參數錯誤! 未選選取乞討對象! 指令範例:\`!beg @AustinBabe 100\``);
    }

    const papa = msg.mentions.users.first();
    if (papa.id == msg.author.id) {
      return msg.channel.send(`自己跟自己乞討是什麼概念? 請開始你的表演`);
    }
    if (isNaN(args[1])) {
      return msg.channel.send(`乞討參數錯誤! 請輸入數字! 指令範例:\`!beg @AustinBabe 100\``);
    }
    ...
    if (amount > papaChips) {
      return msg.channel.send(`你爸爸 \`${papa.username}\` 他好像也沒有錢QQ`);
    }
    ...
    collector.on('collect', (m) => {
      ...
      if (reply.toLocaleLowerCase() == 'yes' || reply.toLocaleLowerCase() == 'y') {
        ...
        return msg.channel.send(`你爸爸 \`${papa.username}\` 給你了\`${amount}\` ${emoji}`);
      } else if (reply.toLocaleLowerCase() == 'no' || reply.toLocaleLowerCase() == 'n') {
        isReply = true;
        return msg.channel.send(`你爸爸 \`${papa.username}\` 拒絕你了`);
      }
    });
    collector.on('end', (collected) => {
      if (!isReply) {
        return msg.channel.send('請求超時!請求已經被自動回絕');
      }
    });
  },
};
